...
def process():
    ...
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    # Save the new 'section' data to the database
    cursor.execute('''
        INSERT INTO complaints (complain_text, category, submitter_name, roll_number, section, predicted_priority, detected_emotion, key_tokens, is_anonymous)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        processed_data['Complain'], processed_data['Category'], processed_data['Name'],
        processed_data['Roll'], processed_data.get('Section'),
        processed_data['Predicted_Priority'], processed_data['Detected_Emotion'],
        ", ".join(processed_data['Key_Tokens']), processed_data['Anonymous']
    ))
    conn.commit()
    conn.close()
